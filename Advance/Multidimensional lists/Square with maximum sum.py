...
for row_index in range(len(matrix) - submatrix_size + 1):
    for col_index in range(len(matrix[row_index]) - submatrix_size + 1):
        current_sum = get_sum_of_submatrix(matrix, row_index, col_index, 2)
        if best_sum < current_sum:
            best_sum = current_sum
            best_row_index = row_index
            best_column_index = col_index

print(f'{matrix[best_row_index][best_column_index]} {matrix[best_row_index][best_column_index + 1]}')
print(f'{matrix[best_row_index + 1][best_column_index]} {matrix[best_row_index + 1][best_column_index + 1]}')
print(f'{best_sum}')
